...
criaRedesDinamc <- function(area){
  # area <- list()
  # area <- areas_raw[[1]]
  colnames(area[[1]])[4] = "harvest"
  colnames(area[[2]])[4] = "harvest"
  colnames(area[[3]])[4] = "harvest"
  colnames(area[[4]])[4] = "harvest"
  colnames(area[[5]])[4] = "harvest"
...
# Learning the dbn
  ...
  dag_natPsoho <- learn_dbn_struc(dt = trainingDBN,
                               size = size,
                               f_dt = trainingDBN,
                               method = "natPsoho")
   
  dag_dmmhc <- learn_dbn_struc(dt = NULL,
                                size = size,
                                f_dt = trainingDBN,
                                method = "dmmhc",
                                intra = F)
...
# If existing, remove arcs from nodes 'harvest_t'
  for(i in 1:(size-1)){
    ...
    if(length(which(arcs(dag_natPsoho)[,"from"] == testing)) > 0){
      rows <- which(arcs(dag_natPsoho)[,"from"] == testing)
      arcs(dag_natPsoho) <- arcs(dag_natPsoho)[-rows,]
    }
    }
  
  for(i in 1:(size-1)){
    testing <-  paste("harvest_t_", i, sep ="")
    if(length(which(arcs(dag_dmmhc)[,"from"] == testing)) > 0){
      rows <- which(arcs(dag_dmmhc)[,"from"] == testing)
      arcs(dag_dmmhc) <- arcs(dag_dmmhc)[-rows,]
    }
    }
...
# Training the network
  dag_natPsoho_fited <- fit_dbn_params(dag_natPsoho, trainingDBN)
  
  dag_dmmhc_fited <- fit_dbn_params(dag_dmmhc, trainingDBN)
  
  # dag_natPsoho
  predict_dag_natPsoho_approx <- forecast_ts(testDBN, dag_natPsoho_fited,
                                   obj_vars = "harvest_t_0", rep = 1,
                                   mode = "approx", ini = 1, len = length(testDBN), 
                                   print_res = T, plot_res = F)
  
  predict_dag_natPsoho_exact <- forecast_ts(testDBN, dag_natPsoho_fited,
                                   obj_vars = "harvest_t_0", rep = 1,
                                   mode = "exact", ini = 1, len = length(testDBN), 
                                   print_res = T, plot_res = F)
  #dag_dmmhc
  predict_dag_dmmhc_approx <- forecast_ts(testDBN, dag_dmmhc_fited,
                                   obj_vars = "harvest_t_0", rep = 1,
                                   mode = "approx", ini = 1, len = length(testDBN),
                                   print_res = T, plot_res = F)
   
   predict_dag_dmmhc_exact <- forecast_ts(testDBN, dag_dmmhc_fited,
                                   obj_vars = "harvest_t_0", rep = 1,
                                   mode = "exact", ini = 1, len = length(testDBN),
                                   print_res = T, plot_res = F)
   ...
   natPsoho_approx =  defaultSummary(metrics_df_natPsoho_approx)
   natPsoho_exact =  defaultSummary(metrics_df_natPsoho_exact)
   dmmhc_approx =  defaultSummary(metrics_df_dmmhc_approx)
   dmmhc_exact =  defaultSummary(metrics_df_dmmhc_exact)
   
   metrics_dags = data.frame(rbind(natPsoho_approx, dmmhc_approx, natPsoho_exact, dmmhc_exact))
...
```{r RASCUNHO - TESTES PARA DOCUMENTAÇÃO}
saida <- criaRedes(areas_dis[[1]][[1]], 1)
saida
area1
...
dinamica_A1 <- criaRedesDinamc(areas_raw[[1]])

dinamica_A2 <- criaRedesDinamc(areas_raw[[2]])

dinamica_A3 <- criaRedesDinamc(areas_raw[[3]]) 

dinamica_A4 <- criaRedesDinamc(areas_raw[[4]]) 

dinamica_A5 <- criaRedesDinamc(areas_raw[[5]])

dinamica_A6 <- criaRedesDinamc(areas_raw[[6]]) 
