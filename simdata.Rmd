...
CriaRedes <- function (areaphase, areatype){
  ...
  if(areatype == 1){
      whitelist = data.frame(from = c("variavel_1", "variavel_2", "variavel_3"), 
                            to = c("colheita", "colheita", "colheita"))
    }else if(areatype == 2){
      whitelist = data.frame(from = c("variavel_1"), 
                            to = c("colheita"))
    }else if(areatype == 3){
      whitelist = data.frame(from = c("variavel_3"), 
                            to = c("colheita"))
    }else if(areatype == 4){
      ...
      # cria rede alg HC com definicao da topologia da rede
      hc_dag <-hc(treino, debug = FALSE, whitelist = whitelist,
         blacklist = blacklist)
      
      # plot rede criada acima
      plot(hc_dag)
      
      # cria rede alg HC sem definicao da topologia da rede
      ...
      # validacao das redes
      return(validaRede(teste, treino, hc_dag_fitted, hc_dag2_fitted))
}
```
...
area1 <- rodaRede__(areas[[1]], 1)
## TESTE
summary(area1)
area1[, "accuracy"]
which.max(area1[1:4,1]) 

select(area1, accuracy, accuracyOVA , precisionOVA)[grep("\\bhc_dag\\b", row.names(area1), fixed = FALSE, value = FALSE),]


write.csv(area1, file = "/home/gui/TCC/R/meu_pacote/saidas/out_area1_++.csv")
...
```{r CriaRedes__}
### Cria redes pra todas as fases fenológicas da área
CriaRedes__ <- function (areaphase, areatype){
  ...
  blacklist = data.frame(from = c("variavel_1","variavel_1",
      ...
      # validacao das redes
      return(validaRede__(teste, treino, hc_dag_fitted, hc_dag_raw_fitted, mmhc_dag_fitted, mmhc_dag_raw_fitted))
}
```

```{r validaRede__}
validaRede__ <- function(test, train, dag_fitted1, dag_fitted2, dag_fitted3, dag_fitted4) {
# Define Target variables (Variables to be predicted)
  pred <-'colheita'
# Evidence variables 
#(Variables that you will give information to the BN to do the prediction)
...
# Metrics Evaluation
  ...
  # print("Matriz de confusão dag 2:")
  # print(metrics2$cmList)
  return(c(results1, metrics1 ,results2 , metrics2, results3, metrics3, results4, metrics4))
}
```


```{r rodaRede__}
rodaRede__ <- function(areadf, areatype){
  area <- list()
  out <- data.frame() 
  nome <- array()
  inome=1
  ...
  for(fase in 1:length(areadf)){
    area[[fase]] <- CriaRedes__(areas[[1]][[fase]], areatype)
    ...
    out <- bind_rows(out, area[[fase]][15])
    nome[inome] <- paste("fase", fase, "mmhc_dag")
    inome = inome+1
    
    out <- bind_rows(out, area[[fase]][20])
