...
grep("\\bfase 1\\b", row.names(area1), fixed = FALSE, value = TRUE)

select(area1, accuracy)[grep("\\bfase 1\\b", row.names(area1), fixed = FALSE, value = FALSE),]
select(area1, accuracy, accuracyOVA , precisionOVA)[grep("\\bfase 1\\b", row.names(area1), fixed = FALSE, value = FALSE),]
select(area1, accuracy, accuracyOVA , precisionOVA)[grep("\\bfase 2\\b", row.names(area1), fixed = FALSE, value = FALSE),]
select(area1, accuracy, accuracyOVA , precisionOVA)[grep("\\bfase 3\\b", row.names(area1), fixed = FALSE, value = FALSE),]
select(area1, accuracy, accuracyOVA , precisionOVA)[grep("\\bfase 4\\b", row.names(area1), fixed = FALSE, value = FALSE),]
select(area1, accuracy, accuracyOVA , precisionOVA)[grep("\\bfase 4\\b", row.names(area1), fixed = FALSE, value = FALSE),]

##gerando gráfico

area1$redes <- rep(c("hc_dag", "hc_dag_raw", "mmhc_dag", "mmhc_dag_raw"), each = 1)
area1$periodo <- rep(c("fase 1", "fase 2", "fase 3", "fase 4", "fase 5"), each = 4)
area1$periodo <- factor(area1$periodo, levels = c("fase 1", "fase 2", "fase 3", "fase 4", "fase 5"))
area1

png("/home/gui/TCC/R/meu_pacote/saidas/grafico.png", width = 1024, height = 768)

plot(ggplot(area1, aes(periodo, accuracy, fill = redes)) +
  ...
  geom_text(aes(label = accuracy),
            ...
            size = 3, color = "black")+
   geom_hline(yintercept = mean(area1$accuracy), linetype = "solid", size = .1)
)
...
select(area2, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 1\\\\b", row.names(area2), fixed = FALSE, value = FALSE),]
select(area2, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 2\\\\b", row.names(area2), fixed = FALSE, value = FALSE),]
select(area2, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 3\\\\b", row.names(area2), fixed = FALSE, value = FALSE),]
select(area2, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 4\\\\b", row.names(area2), fixed = FALSE, value = FALSE),]
select(area2, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 4\\\\b", row.names(area2), fixed = FALSE, value = FALSE),]


area3 <- rodaRede(areas_dis[[3]], 3)

select(area3, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 1\\\\b", row.names(area3), fixed = FALSE, value = FALSE),]
select(area3, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 2\\\\b", row.names(area3), fixed = FALSE, value = FALSE),]
select(area3, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 3\\\\b", row.names(area3), fixed = FALSE, value = FALSE),]
select(area3, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 4\\\\b", row.names(area3), fixed = FALSE, value = FALSE),]
select(area3, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 4\\\\b", row.names(area3), fixed = FALSE, value = FALSE),]
...
select(area4, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 1\\\\b", row.names(area4), fixed = FALSE, value = FALSE),]
select(area4, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 2\\\\b", row.names(area4), fixed = FALSE, value = FALSE),]
select(area4, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 3\\\\b", row.names(area4), fixed = FALSE, value = FALSE),]
select(area4, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 4\\\\b", row.names(area4), fixed = FALSE, value = FALSE),]
select(area4, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 4\\\\b", row.names(area4), fixed = FALSE, value = FALSE),]
...
select(area5, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 1\\\\b", row.names(area5), fixed = FALSE, value = FALSE),]
select(area5, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 2\\\\b", row.names(area5), fixed = FALSE, value = FALSE),]
select(area5, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 3\\\\b", row.names(area5), fixed = FALSE, value = FALSE),]
select(area5, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 4\\\\b", row.names(area5), fixed = FALSE, value = FALSE),]
select(area5, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 4\\\\b", row.names(area5), fixed = FALSE, value = FALSE),]
...
select(area6, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 1\\\\b", row.names(area6), fixed = FALSE, value = FALSE),]
select(area6, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 2\\\\b", row.names(area6), fixed = FALSE, value = FALSE),]
select(area6, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 3\\\\b", row.names(area6), fixed = FALSE, value = FALSE),]
select(area6, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 4\\\\b", row.names(area6), fixed = FALSE, value = FALSE),]
select(area6, accuracy, accuracyOVA , precisionOVA)[grep("\\\\bfase 4\\\\b", row.names(area6), fixed = FALSE, value = FALSE),]
...
## zona de testes 
  ...
  todas_fases_pre <- data.frame(X_1 = c(1,2,3,4,5), 
                               X_2 = c(1,2,3,4,5),
                               X_3 = c(1,2,3,4,5),
                               colheita  = c(1,2,3,4,5))
  
  f_todas_fases_pre <- fold_dt(todas_fases_pre, 5)
  
  todas_fases_pre_p <-matrix(0, nrow = 1000, ncol = 20)
  todas_fases_pre_p <- as.data.frame(todas_fases_pre_p)
  colnames(todas_fases_pre_p) <- colnames(f_todas_fases_pre)
  colnames(todas_fases_pre_p)
  todas_fases_pre_p
  
  todas_fases_pre_p[,1:4] = areas_raw[[1]][[5]]
  todas_fases_pre_p[,5:8] = areas_raw[[1]][[4]]
  todas_fases_pre_p[,9:12] = areas_raw[[1]][[3]]
  todas_fases_pre_p[,13:16] = areas_raw[[1]][[2]]
  todas_fases_pre_p[,17:20] = areas_raw[[1]][[1]]
  todas_fases_pre_p
  
  todas_fases_pre_p <- as.data.table(todas_fases_pre_p)
  
#Valid methods are: dmmhc, psoho, natPsoho
# dmmhc: Error in bnlearn::node.ordering(structure): the graph is only partially directed.
  natPsoho_f_dt_net <- learn_dbn_struc(dt = todas_fases_pre_p, 
                              size = 5, 
                              f_dt = todas_fases_pre_p,
                              method = "natPsoho")
  
  plot(natPsoho_f_dt_net)
  print(natPsoho_f_dt_net)
  fitted_natPsoho_f_dt_net <- fit_dbn_params(natPsoho_f_dt_net, todas_fases_pre_p)
  natPsoho_predict_dt<- forecast_ts(todas_fases_pre_p, fitted_natPsoho_f_dt_net,
                                      obj_vars = "colheita_t_0", rep = 2, mode = "exact",
                                      ini = 1, len = 10, print_res = T, plot_res = F)
  natPsoho_predict_dt
  
  
  
## teste disponível no repositório do pacote dbnR
...
```{r teste_novo dbnR}
teste_novo <- rbind(areas_raw[[1]][[5]][1,], 
                      areas_raw[[1]][[4]][1,],
                      areas_raw[[1]][[3]][1,], 
                      areas_raw[[1]][[2]][1,],
                      areas_raw[[1]][[1]][1,])
  ...
  teste_novo_f <- fold_dt(teste_novo, 5)
  colnames(teste_novo_f)

  natPsoho_teste_novo <- learn_dbn_struc(dt = teste_novo,
                              size = 5,
                              f_dt = teste_novo_f,
                              method = "natPsoho")
  plot(natPsoho_teste_novo)
  fit_natPsoho_teste_novo <- fit_dbn_params(natPsoho_teste_novo, teste_novo_f)
  predict_teste_novo<- forecast_ts(teste_novo_f, fit_natPsoho_teste_novo,
                                      obj_vars = "colheita_t_0", rep = 2, mode = "exact",
                                      ini = 1, len = 1, print_res = T, plot_res = F)
  predict_teste_novo
```
