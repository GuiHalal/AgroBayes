...
# debug(BuildSimulationData)
  ...
  names(areas_list) <- names(areas_list)
  
  for(area in  1:length(areas_list)){
    for(fase in 1:length(areas_list[[1]])){
      for(var in 1:dim(areas_list[[1]][[fase]])[2]){
...
blacklist = data.frame(from = c("variavel_1","variavel_1",
 ...
 arc_set = as.matrix(whitelist)
  ...
  hc_dag <-hc(areas_teste[[6]][[4]], debug = FALSE, whitelist = whitelistA6, blacklist = blacklist)
  #blacklist(hc_dag)
  plot(hc_dag) 
  bn_hc_dag = bn.fit(hc_dag, areas_teste[[6]][[5]])
  bn_hc_dag[[4]]
